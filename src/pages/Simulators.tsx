...
export default function simulators() {
  ...
  const { simulators, isLoading, saveDebt, isSaving, deleteDebt } = usesimulators();
  ...
  const handleAnalyze = async () => {
    const isValid = await trigger(["value", "downPayment", "rate", "installments", "interestType", "date", "ratePeriod"]);
    if (!isValid) return;
    ...
    const chartData = generateChartData(financedpayment, effectiveMonthlyRate, months, isCompound, data.date);
  ...
  const onSubmit = async (data: DebtForm) => {
    ...
    try {
      ...
      } as DebtData & { date?: string | null, interestType?: string, installments?: number, ratePeriod?: string, downPayment?: number });
  ...
  const totalParcelamentos = simulators?.reduce((acc, d) => {
    const financiado = (d.value || 0) - ((d as any).downPayment || 0);
    return acc + financiado + (d.payment || 0);
  }, 0) || 0;
  
  const filteredsimulators = simulators?.filter(d => filterStatus === "Todos" || d.status === filterStatus) || [];
      ...
      <main className={`relative z-10 transition-all duration-300 p-4 md:p-8 space-y-8 ${collapsed ? "ml-[72px]" : "ml-[72px] md:ml-[260px]"}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="relative overflow-hidden bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-[#77127B]/20 rounded-3xl p-6 shadow-lg shadow-[#77127B]/5 group hover:border-[#77127B]/40 transition-colors">
            ...
            <p className="text-3xl font-black text-[#77127B] dark:text-white">{simulators?.length || 0}</p>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="relative overflow-hidden bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-[#426DA9]/20 rounded-3xl p-6 shadow-lg shadow-[#426DA9]/5 group hover:border-[#426DA9]/40 transition-colors">
            ...
            <p className="text-3xl font-black text-[#426DA9] dark:text-white">{simulators?.filter((d) => d.status === "Ativa").length || 0}</p>
          </motion.div>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-6">
              ...
              <div className="lg:col-span-4">
                ...
                <input {...register("creditor")} placeholder="Ex: Empréstimo, carro, casa própria, etc." className={fieldClass} onChange={() => setAnalysisData(null)} />
              ...
              {/* Linha de Baixo */}
              <div className="lg:col-span-3">
                <label className="text-[11px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-2 block">Início</label>
