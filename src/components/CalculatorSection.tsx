...
import { persistFinancialData } from "@/lib/financialPersistence";
...
const steps = [
  {
    key: "divida",
    title: "Dívida Ativa",
    question: "Você possui alguma dívida ativa?",
    description: "Dívida ativa é qualquer valor que você deve a terceiros.",
    image: "/divida.png",
    multiple: true,
  },
  {
...
export default function CalculatorSection() {
  ...
  async function saveToSupabase(finalData: FinancialData) {
    ...
    try {
      await persistFinancialData(user.id, finalData);
    } catch (error) {
  ...
  if (showResults) {
    return (
      <div className="w-full relative animate-in fade-in duration-500">
        {/* Adicionado mt-10 para desgrudar de cima, mb-3 para aproximar da section e flex-wrap com gap-4 */}
        ...
        <div className="w-full md:w-7/12 p-8 md:p-12 flex flex-col justify-center min-h-[500px]">
          <AnimatePresence mode="wait">
            ...
            >
              <motion.div
                layout
                className="mb-3 text-sm font-extrabold text-[#C1188B] uppercase tracking-widest bg-[#C1188B]/10 dark:bg-[#C1188B]/20 inline-block px-4 py-1.5 rounded-full w-max"
              >
              ...
              <motion.h3
                layout
                className="text-3xl font-extrabold text-[#1D4F91] dark:text-white mb-3 tracking-tight"
              >
                {step.title}
              </motion.h3>
              <motion.p
                layout
                className="text-gray-500 dark:text-gray-300 mb-8 text-lg leading-relaxed"
              >
                {step.question}
              </motion.p>
              ...
              <div className="flex flex-col flex-grow justify-end">
                {items.length > 0 && (
                  <motion.div
                    layout
                    className="flex flex-wrap gap-2.5 mb-6 max-h-[160px] overflow-y-auto pr-2 pb-2 custom-scrollbar"
                  >
                    <AnimatePresence>
                      {items.map((item, i) => (
                        <motion.div
                ...
                <motion.div
                  layout
                  className="flex flex-col sm:flex-row gap-4 mt-auto mb-6"
                >
                  <div className="relative flex-1 group">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 font-bold text-lg group-focus-within:text-[#E80070] dark:group-focus-within:text-[#E80070] transition-colors">
                      R$
                    </span>
                    <input
                      value={inputValue}
              ...
              <motion.div
                layout
                className="flex justify-between items-center pt-6 border-t border-gray-100 dark:border-gray-700 mt-2 gap-4"
              >
                <MagneticButton
                  onClick={handleBack}
