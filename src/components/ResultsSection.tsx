...
export default function ResultsSection({ data, onSave }: Props) {
  ...
  const [valores, setValores] = useState(buildValoresFromData);
  ...
  useEffect(() => {
    if (!isEditing) {
      setValores(buildValoresFromData());
    }
  }, [data, isEditing]);
    ...
    const rawData = [
      {
        id: "Renda Fixa",
        label: "Renda Fixa",
        value: isChartMounted ? valores.rendaFixa : 0.01,
        color: COLORS.lightBlue,
      },
      {
        id: "Renda Variável",
        label: "Renda Variável",
        value: isChartMounted ? valores.rendaVariavel : 0.01,
        color: COLORS.darkBlue,
      },
      {
        id: "Gastos Fixos",
        label: "Gastos Fixos",
        value: isChartMounted ? valores.gastosFixos : 0.01,
        color: COLORS.magenta,
      },
      {
        id: "Gastos Variáveis",
        label: "Gastos Variáveis",
        value: isChartMounted ? valores.gastosVariaveis : 0.01,
        color: COLORS.magenta,
      },
      {
        id: "Investimentos",
        label: "Investimentos",
        value: isChartMounted ? valores.investimentos : 0.01,
        color: COLORS.purple,
      },
    ];
  ...
  const barData = useMemo(
    () => [
      {
        name: "Renda",
        value: isChartMounted ? rendaTotal : 0,
        color: COLORS.darkBlue,
      },
      {
        name: "G. Fixos",
        value: isChartMounted ? valores.gastosFixos : 0,
        color: COLORS.magenta,
      },
      {
        name: "G. Var.",
        value: isChartMounted ? valores.gastosVariaveis : 0,
        color: COLORS.magenta,
      },
      {
        name: "Invest.",
        value: isChartMounted ? valores.investimentos : 0,
        color: COLORS.purple,
      },
      {
        name: "Saldo",
        value: isChartMounted ? saldo : 0,
        color: saldo >= 0 ? COLORS.lightBlue : "#ef4444",
      },
      {
        name: "Dívidas",
        value: isChartMounted ? valores.dividas : 0,
  ...
  const summaryItems = [
    {
      ...
      label: "Renda Total",
      value: rendaTotal,
      color: "text-[#1D4F91] dark:text-[#426DA9]",
      bg: "bg-[#1D4F91]/10 dark:bg-[#1D4F91]/20",
    ...
    },
    {
      id: "gastosFixos",
      label: "Gastos Fixos",
      value: valores.gastosFixos,
      color: "text-[#E80070]",
      bg: "bg-[#E80070]/10 dark:bg-[#E80070]/20",
    ...
    },
    {
      ...
      color: "text-[#C1188B]",
      bg: "bg-[#C1188B]/10 dark:bg-[#C1188B]/20",
    ...
    },
    {
      ...
      color: "text-[#77127B]",
      bg: "bg-[#77127B]/10 dark:bg-[#77127B]/20",
      icon: <Target size={22} />,
      editable: true,
    },
    {
      ...
      label: "Saldo Livre",
      value: saldo,
      color: saldo >= 0 ? "text-[#426DA9]" : "text-[#E80070]",
      bg:
        saldo >= 0
          ? "bg-[#426DA9]/10 dark:bg-[#426DA9]/20"
          : "bg-[#E80070]/10 dark:bg-[#E80070]/20",
      icon: <Wallet size={22} />,
      editable: false,
    },
  ];
    ...
    <div className="w-full max-w-6xl mx-auto flex flex-col gap-10 transition-colors duration-300">
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <motion.div
          ...
          className="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-700 flex flex-col items-center relative transition-colors duration-300 group hover:shadow-2xl"
        >
          ...
          <h3 className="text-2xl font-extrabold text-gray-800 dark:text-white mb-6 w-full text-left tracking-tight">
          ...
          <div className="flex flex-wrap justify-center gap-x-6 gap-y-3 mt-6 w-full px-4">
            {isChartMounted &&
              pieData.map((item) => (
                <div
                  key={item.id}
                  className="flex items-center gap-2.5 text-sm font-semibold text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors cursor-default"
                >
                  <span
      ...
      <motion.div
        ...
        className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl border border-gray-100 dark:border-gray-700 p-8 lg:p-10 transition-colors duration-300 relative overflow-hidden"
      >
        ...
        <div className="flex justify-between items-center mb-10 relative z-10">
          <h3 className="text-3xl font-extrabold text-gray-800 dark:text-white flex items-center gap-4 tracking-tight">
            <div className="p-3 bg-[#426DA9]/10 dark:bg-[#1D4F91]/20 rounded-2xl">
              <Wallet
                className="text-[#426DA9] dark:text-[#426DA9]"
                size={36}
              />
            </div>
            Resumo Financeiro
          ...
          {onSave ? (
            <button
              ...
              disabled={isSaving}
              className={`flex items-center gap-2.5 px-6 py-3 rounded-2xl transition-all duration-300 font-bold text-sm shadow-md hover:shadow-lg disabled:cursor-not-allowed disabled:opacity-70 ${
                isEditing
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-5 relative z-10">
          {summaryItems.map((item) => (
            ...
            >
              <div
                className={`mb-3 p-3 bg-white/50 dark:bg-black/20 rounded-2xl shadow-sm ${item.color}`}
              >
                {item.icon}
              </div>
              ...
              {isEditing && item.editable ? (
                <div className="w-full relative mt-1 bg-white dark:bg-gray-800 rounded-xl px-2 py-1 shadow-inner border dark:border-gray-600">
                  <span
                    className={`absolute left-3 top-1/2 -translate-y-1/2 text-sm font-bold ${item.color} opacity-80`}
                  >
                    R$
                  </span>
                  <input
                    ...
                    value={item.value || ""}
                    onChange={(e) =>
                      handleValueChange(item.id, parseFloat(e.target.value))
                    }
                    disabled={isSaving}
                ...
                <p
                  className={`font-extrabold text-xl ${item.color} drop-shadow-sm`}
                >
...
function AlertsPanel({
  ...
  if (fixosPct > 60)
    alerts.push({
      msg: `Atenção: Seus gastos fixos comprometem ${fixosPct.toFixed(0)}% da sua renda. O ideal é manter até 50%.`,
      level: "danger",
    });
  else if (fixosPct >= 51)
    alerts.push({
      msg: "Cuidado: Seus gastos fixos estão próximos do limite recomendado (50%).",
      level: "warning",
    });

  if (varPct > 40)
    alerts.push({
      msg: `Atenção: Seus gastos variáveis representam ${varPct.toFixed(0)}% da renda. O ideal é focar em 30%.`,
      level: "danger",
    });
  else if (varPct >= 31)
    alerts.push({
      msg: "Cuidado: Tente reduzir um pouco os gastos variáveis (limite superado de 30%).",
      level: "warning",
    });

  if (saldo < 0) {
    alerts.push({
      msg: "Urgente: Você está fechando no vermelho. Priorize cortar gastos imediatamente e negocie dívidas.",
      level: "danger",
    });
  }
      ...
      <div className="bg-gradient-to-r from-[#1D4F91] to-[#426DA9] p-8 rounded-3xl shadow-xl flex items-center gap-6 text-white overflow-hidden relative">
        ...
        <div>
          ...
          <p className="font-medium text-white/90 text-lg leading-relaxed">
            Seus gastos parecem equilibrados em relação à sua renda. Continue
            assim e não esqueça de investir!
          </p>
        </div>
      </div>
    );
      ...
      {alerts.map((a, i) => (
        <div
          ...
          className={`p-6 md:p-8 rounded-3xl shadow-lg flex items-start gap-5 font-bold border-l-[10px] bg-white dark:bg-gray-800 transition-all hover:scale-[1.01] ${
            a.level === "danger" ? "border-[#E80070]" : "border-[#C1188B]"
          }`}
        >
          <div
            className={`p-3 rounded-2xl ${a.level === "danger" ? "bg-[#E80070]/10 text-[#E80070]" : "bg-[#C1188B]/10 text-[#C1188B]"}`}
          >
            <AlertTriangle size={32} />
          </div>
          <span className="text-lg md:text-xl text-gray-800 dark:text-gray-200 self-center leading-snug">
            {a.msg}
          </span>
        </div>
      ))}
