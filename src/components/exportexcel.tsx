...
const C = {
  ...
  EXPENSE_BG:  "FDECEA",
  BLACK:       "000000",
  INVEST_BG:   "E3F2FD", // Fundo azul claro para investimento
};
...
const STY = {
  ...
  tipo: (type: string) => {
    let color = C.EXPENSE_RED;
    let bg = C.EXPENSE_BG;
    if (type === "Renda") { color = C.INCOME_GRN; bg = C.INCOME_BG; }
    if (type === "Investimento") { color = C.DARK_BLUE; bg = C.INVEST_BG; }
  ...
  val: (type: string, bg: string) => {
    let color = C.EXPENSE_RED;
    if (type === "Renda") color = C.INCOME_GRN;
    if (type === "Investimento") color = C.DARK_BLUE;
...
export default function ExportExcelButton({ data }: Props) {
  function exportToExcel() {
    ...
    const investimentos = data.filter((t) => t.type === "Investimento");
    ...
    // Saldo Final desconta Despesas e Investimentos
    ...
    const ws1: any = {
      ...
      "!rows":   [],
      "!cols":   [
        { wpx: 36  }, { wpx: 130 }, { wpx: 210 }, { wpx: 140 }, { wpx: 110 }, { wpx: 36  },
      ],
    };

    const rh = (px: number) => ({ hpx: px });
    const M1 = (sr: number, sc: number, er: number, ec: number) => ws1["!merges"].push({ s:{r:sr-1,c:sc-1}, e:{r:er-1,c:ec-1} });
    const wc1 = (r: number, c: number, v: any, sty: any, fmt?: string) => writeCell(ws1, r, c, v, sty, fmt);
    ...
    ws1["!rows"].push(rh(22));
    ["#","Tipo","Categoria","Valor (R$)","Data",""].forEach((h, i) => wc1(9, i + 1, h, STY.colHead));
    ws1["!rows"].push(rh(6));
    ...
    data.forEach((item, i) => {
      ...
      if (rawDate) {
        ...
        wc1(row, 5, !isNaN(d.getTime()) ? d : "", STY.dateCell(bg), DATE_FMT);
      } else {
    ...
    // SHEET 2 Dashboard
    const ws2: any = { "!merges": [], "!rows": [], "!cols": [ { wpx: 36 }, { wpx: 200 }, { wpx: 130 }, { wpx: 140 }, { wpx: 36 } ] };
    const wc2 = (r: number, c: number, v: any, sty: any, fmt?: string) => writeCell(ws2, r, c, v, sty, fmt);
    const fr2 = (r: number, cols: number[], sty: any) => fillRow(ws2, r, cols, sty);
    const M2 = (sr: number, sc: number, er: number, ec: number) => ws2["!merges"].push({ s:{r:sr-1,c:sc-1}, e:{r:er-1,c:ec-1} });
    ...
    ["#","Categoria","Tipo","Total (R$)",""].forEach((h, i) => wc2(5, i + 1, h, STY.colHead));
    ...
    wc2(totRow, 4, data.reduce((a, t) => a + (Number(t.value) || 0), 0), STY.totalVal, BRL);
    M2(totRow,2, totRow,3);

    const foot2 = totRow + 3;
    ws2["!rows"].push(rh(10), rh(10), rh(24));
    wc2(foot2, 1, "© DebtView  •  Análise por Categoria", STY.footer); M2(foot2,1, foot2,5);
