...
// ── Cell writer ───────────────────────────────────────────────────────────
// Converts Date → Excel serial to avoid xlsx-js-style "d" type issues
function writeCell(
  ws: any,
  row: number,
  col: number,
  ...
  function exportToExcel() {
    ...
    data.forEach((item, i) => {
      ...
      if (rawDate) {
        ...
        if (!isNaN(d.getTime())) {
          wc1(row, 5, d,  STY.dateCell(bg), DATE_FMT);
        } else {
    ...
    toast({
      ...
      description: "Seu relatório DebtView foi gerado.",
    });
  }
